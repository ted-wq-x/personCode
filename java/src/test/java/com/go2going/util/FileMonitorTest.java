...
public class FileMonitorTest {
    ...
    public static void setUpBeforeClass() throws Exception
    {
        ...
        _scanner = new FileMonitor();
        _scanner.addScanDir(_directory);
        _scanner.setScanInterval(0);
        _scanner.addListener(new FileMonitor.DiscreteListener()
        {
            @Override
            public void fileRemoved(String filename) throws Exception
            {
                _queue.add(new Event(filename, FileMonitor.Notification.REMOVED));
            }

            @Override
            public void fileChanged(String filename) throws Exception
            {
                _queue.add(new Event(filename, FileMonitor.Notification.CHANGED));
            }

            @Override
            public void fileAdded(String filename) throws Exception
            {
                _queue.add(new Event(filename, FileMonitor.Notification.ADDED));
            }
        });
        _scanner.addListener(new FileMonitor.BulkListener()
        {
            @Override
    ...
    {
        _scanner.doStop();
        delete(_directory.getPath());
    }

    static class Event
    {
        String _filename;
        FileMonitor.Notification _notification;

        public Event(String filename, FileMonitor.Notification notification)
        {
            _filename=filename;
    ...
    public void testAddedChangeRemove() throws Exception
    {
        ...
        Assert.assertEquals(FileMonitor.Notification.ADDED,event._notification);
        ...
        Assert.assertTrue(event!=null);
        Assert.assertEquals(_directory+"\\a1",event._filename);
        Assert.assertEquals(FileMonitor.Notification.CHANGED,event._notification);
        Assert.assertTrue(_queue.isEmpty());
        ...
        Assert.assertTrue(event!=null);
        Assert.assertEquals(_directory+"\\a1",event._filename);
        Assert.assertEquals(FileMonitor.Notification.REMOVED,event._notification);
        Assert.assertTrue(_queue.isEmpty());
    ...
    public void testSizeChange() throws Exception
    {
        ...
        Event event = _queue.poll();
        Assert.assertTrue(event!=null);
        Assert.assertEquals(_directory+"\\tsc0",event._filename);
        Assert.assertEquals(FileMonitor.Notification.ADDED,event._notification);
    ...
    private static void delete(String string) throws IOException
    {
    ...
    private static void touch(String string) throws IOException
    {
        File file = new File(_directory,string);
        if (file.exists())
